...
for(site in sites){
  ...
  hmmIBD <- read.delim(sprintf('../pf3k_chimeric_data/hmmIBD_%s.hmm.txt',site)) 
  ...
  comparisons_of_interest <- paste(child = rep(children_names,2), 
                                   as.vector(do.call(rbind,(strsplit(children_names, split = ":")))), sep = ' ')
  ...
  # Create comparison names for isoRelate and Haplotype for look up with comparisons of interest
  ...
  save(Results, file = sprintf('../pf3k_chimeric_data/Results_comparisons_of_interest_%s.RData', site))
  save(comparisons_of_interest, file = sprintf('../pf3k_chimeric_data/comparisons_of_interest_%s.RData', site))
  save(isoRelate_assignment, file = sprintf('../pf3k_chimeric_data/isoRelate_assignment_%s.RData', site))
  save(hmmIBD_assignment, file = sprintf('../pf3k_chimeric_data/hmmIBD_assignment_%s.RData', site))
}